...
from agents import get_available_agents, set_agent, get_current_agent, get_metrics, update_metrics, reset_metrics
...
def get_model_info():
    ...
    return _json_safe({
        "loaded": True,
        "source": "simple_agent",
        "agent_name": current_agent.name if hasattr(current_agent, 'name') else 'unknown',
        "strategy": current_agent.strategy if hasattr(current_agent, 'strategy') else 'unknown'
    })
...
def get_agents():
    ...
    return _json_safe({
        ...
        "current": get_current_agent().name if hasattr(get_current_agent(), 'name') else 'unknown'
    })
